 ...
 */
    ...
    /**
     * Gets all property with pattern @SomeAnnotation() from a determinated method of a given class
     *
     * @param  string $className             class name
     ...
     */
    public static function getPropertiesAnnotations($className)
    {
        ...
        foreach ($props as $prop) {
            $out[$prop->getName()] = self::getPropertyAnnotationsFromReflection($prop);
        }
        return $out;
    }
    ...
    {
        return self::getPropertyAnnotationsFromReflection(new \ReflectionProperty($className, $propertyName));
    }

    public static function getPropertyAnnotationsFromReflection(\ReflectionProperty $property)
    {
        return self::parseAnnotations($property->getDocComment());
    }
    ...
    protected static function parseAnnotations($docblock)
    {
        ...
        if (preg_match_all('/@(?<name>[A-Za-z_-]+)[ \t]*(?<args>.*)$/m', $docblock, $matches)) {
